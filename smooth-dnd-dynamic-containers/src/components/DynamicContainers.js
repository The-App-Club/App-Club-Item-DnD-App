  ...
  render() {
    return (
      <div style={{ display: 'flex', justifyContent: 'stretch', height: '100%' }}>
        <div className="dynamic-left-pane">{this.renderContainer('items1', () => this.state.items1)}</div>
        <div className="dynamic-right-pane">
          <div className="dynamic-menu-container">
            <div className="popup-container-button" onMouseEnter={() => this.setState({ popup1Open: true })} onMouseLeave={() => this.setState({ popup1Open: false })}>
              Make Container Visible
              <div className={`popup-container ${this.state.popup1Open ? 'open' : ''}  ${constants.preventAutoScrollClass}`}>{this.renderContainer('items2', () => this.state.items2)}</div>
            </div>
            <div className="popup-container-button" onMouseEnter={() => this.setState({ popup2Open: true })} onMouseLeave={() => this.setState({ popup2Open: false })}>
              Mount New Container
              {this.state.popup2Open ? <div className={`popup-container ${this.state.popup2Open ? 'open' : ''}`}>{this.renderContainer('items3', () => this.state.items3)}</div> : null}
            </div>
          </div>
          <div className="dynamic-right-content">{this.renderContainer('items4', () => this.state.items4)}</div>
        </div>
      </div>
    );
  }
