...
function Group() {
  const [items1, setItems1] = useState(generateItems(15, (i) => ({ id: '1' + i, data: `Draggable 1 - ${i}` })));
  ...
  const [items4, setItems4] = useState(generateItems(15, (i) => ({ id: '4' + i, data: `Draggable 4 - ${i}` })));
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'stretch', marginTop: '50px', marginRight: '50px' }}>
      <div style={groupStyle}>
        <Container groupName="1" getChildPayload={(i) => items1[i]} onDrop={(e) => handleDrop(e, 'items1')}>
          {items1.map((p) => {
            return (
              <Draggable key={p.id}>
      ...
      <div style={groupStyle}>
        <Container groupName="1" getChildPayload={(i) => items4[i]} onDrop={(e) => handleDrop(e, 'items4')}>
          {items4.map((p) => {
            return (
              <Draggable key={p.id}>
                <div className="draggable-item">{p.data}</div>
